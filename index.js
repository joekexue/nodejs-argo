...
const UUID = process.env.UUID || '18393c1b-57ab-40ad-a719-50d0b0e7f84a'; // 使用哪吒v1,在不同的平台运行需修改UUID,否则会覆盖
...
const ARGO_DOMAIN = process.env.ARGO_DOMAIN || 'galaxy.5980268.xyz';          // 固定隧道域名,留空即启用临时隧道
const ARGO_AUTH = process.env.ARGO_AUTH || 'eyJhIjoiYzAwYWY1MDhjNzBmMGM4NWQxNmEyY2EyMDAwZTZjYTgiLCJ0IjoiNWJhYjlhYjItZGQxZS00Y2EzLWIzNmYtNGQxZjg0MWNjNGM3IiwicyI6IllXUTRaV1U1TnpFdE5XRTBNaTAwWkRWa0xXSmtNak10TVRobE9EVXdZbVE1WVROayJ9';              // 固定隧道密钥json或token,留空即启用临时隧道,json获取地址：https://fscarmen.cloudflare.now.cc
const ARGO_PORT = process.env.ARGO_PORT || 38465;            // 固定隧道端口,使用token需在cloudflare后台设置和这里一致
